...
#pragma omp parallel for private(j, sum, sqr_sum, l1, l2) reduction(+:violations, steps, stepsum) schedule(static) 
  for (j = 0; j < p; j++) {
    if (e1[j] == 0) {
      ...
      sum = 0.0;
      sqr_sum = 0.0;
      current_sample = nsample = max(50, n / 1000);
      ...
      do {        
        current_sample = nsample;
         for (int i = start_pos[j]; i < start_pos[j] + current_sample; i++) {
        ...
        start_pos[j] = (start_pos[j]+3*current_sample < n) ?  start_pos[j]+current_sample : 0;        
        nsample = nsample + current_sample; 
