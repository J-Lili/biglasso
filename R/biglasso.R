...
biglasso <- function(X, y, row.idx = 1:nrow(X),
                     ...
                     screen = c("SSR", "SEDPP", "SSR-BEDPP", "SSR-BEDPP-turbo", "SSR-Slores", "SSR-Slores-Batch",
                                "SSR-Dome", "None", "turbo","NS-NAC", "SSR-NAC", "SEDPP-NAC",
    ...
    time <- system.time(
      {
        switch(screen,
               "turbo" = {
                 res <- .Call("cdfit_gaussian_turbo", X@address, yy, as.integer(row.idx-1),
               ...
               "SSR-BEDPP-turbo" = {
                 res <- .Call("cdfit_gaussian_hsr_bedpp_turbo", X@address, yy, as.integer(row.idx-1),
                              lambda, as.integer(nlambda), as.integer(lambda.log.scale),
                              lambda.min, alpha,
                              as.integer(user.lambda | any(penalty.factor==0)),
                              eps, as.integer(max.iter), penalty.factor,
